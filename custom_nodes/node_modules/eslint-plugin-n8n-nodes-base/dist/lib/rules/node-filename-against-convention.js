...
var node_filename_against_convention_default = import_utils.utils.createRule({
  ...
  meta: {
    type: "problem",
    docs: {
      description: "`name` in node class description must match the node filename without the `.node.ts` suffix. Example: If `description.name` is `Test`, then filename must be `Test.node.ts`.",
      recommended: "strict"
    },
    schema: [],
    messages: {
  ...
  create(context) {
    return {
      ObjectExpression(node) {
        ...
        if (!name)
          return;
        const actual = import_utils.utils.getNodeFilename(
